...
| Local package | Handles MCP over stdio, loads one configured credential — a platform token or an account-restricted token — discovers the paper tool schemas that credential is offered, and forwards calls to the backend. |
...
The account-record interface returns stored cash, position quantities, cost basis, orders, and immutable fills. Option-chain tools return contract metadata only. Account tools do not expose current market marks or P&L. The competition tools return stored standings and scores — the equity, return, rank and valuation timestamp last recorded for each entry, with a stale flag — and the member tools return a fellow participant's stored holdings and activity under the backend's membership rule. Every one of those numbers is stored rather than fetched: no read here prices anything. The owner dashboard uses separately persisted valuation snapshots with source and age labels.

The bridge allows only the 20 named paper tools listed in [TOOLS.md](TOOLS.md). It does not expose arbitrary HTTP requests or add tools to fund accounts, reset deposits, or edit prices. Read-only and trade permissions are enforced on the server even if a client ignores tool annotations.
...
Use a paper token: a platform token, or an account-scoped one for an agent restricted to a single account. The token is loaded once when the process starts, either from `PAPER_TRADING_TOKEN_FILE` or `PAPER_TRADING_TOKEN`, with both together rejected. File mode checks on POSIX systems prevent a token file readable by other users; Windows file access is controlled by the user's ACLs. A replacement token takes effect after restarting the process.
...
Without a configured token, the bridge exposes sign-in tools and uses the backend's dedicated browser approval flow. A configured token of either kind replaces that flow rather than supplementing it: the sign-in tools are not offered, so no browser approval is involved. This is a local stdio onboarding protocol, not the MCP HTTP OAuth standard. The human logs in using an existing KH browser session. The bridge generates a random management credential and sends its SHA-256 hash in a short-lived approval request. The browser receives only a confirmation code; the polling secret stays local. Approval binds the hash to the authenticated user for the selected, server-bounded lifetime (24 hours by default). Optional token management requires separate consent.

Management grants are memory-only in the local process, database-checked for expiry/revocation on every backend call, restricted to the MCP endpoint, and never carry admin authority. Tokens of both kinds are saved in owner-only local files. Default child tokens depend on the parent grant. Explicitly consented independent tokens have their own server-bounded expiry and remain usable after session revocation. The backend retains their issuer for auditing. No user signup, refresh-token flow, or long-lived management credential storage is implemented. Existing account-token authentication stays supported.
