...
`paper_create_token` saves the one-time child token in a private file under `~/.config/kh-paper-trading/tokens/` (or `PAPER_TRADING_CREDENTIAL_DIR`). Its tool result contains metadata and the file path, not the secret. Configure another local agent with `PAPER_TRADING_TOKEN_FILE`. By default these credentials expire with the approving session. For unattended agents, separately approve independent-token creation and call `paper_create_token` with `independent: true` and `ttlDays` (365 days by default, bounded by backend policy). Independent tokens survive session expiry, browser sign-out, and local process restarts when loaded from the saved file. They cannot grant management permissions or issue other tokens. File-based handoff currently targets agents on the same machine.
